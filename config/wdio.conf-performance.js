		...
		if (process.env.TV_IP && process.argv.find(arg => arg.includes('tv.conf'))) {
			...
			try {
				if (process.platform === 'win32') {
					// Windows
					const chromeVersion = /\d+/.exec(execSync('wmic datafile where "name=\'C:\\\\Program Files\\\\Google\\\\Chrome\\\\Chrome-bin\\\\chrome.exe\'" get Version /value').toString());
		...
		opts,
		{
			...
			capabilities: [{
				...
				'goog:chromeOptions': visibleBrowser ? {
					binary: 'C:/Program Files/Google/Chrome/Chrome-bin/chrome.exe'
				} : {
